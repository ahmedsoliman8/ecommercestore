            ...
            <div class="row">
                ...
                <div class="col-lg-3 order-2 order-lg-1">
                    ...
                    @foreach($shop_categories_menu as $shop_category)
                        <div class="py-2 px-4 {{$loop->first?'bg-dark text-white':'bg-light'}}  mb-3"><strong class="small text-uppercase font-weight-bold">{{$shop_category->name}}</strong></div>
                        <ul class="list-unstyled small text-muted pl-lg-4 font-weight-normal">
                            @forelse($shop_category->appearedChildren as $sub_shop_category)
                                <li class="mb-2"><a href="{{route('frontend.shop',$sub_shop_category->slug)}}" class="reset-anchor" >{{$sub_shop_category->name}}</a></li>
                            @empty
                            @endforelse
                        </ul>
                    @endforeach
                    ...
                    <ul class="list-unstyled small text-muted pl-lg-4 font-weight-normal">
                        @forelse($shop_tags_menu as  $shop_tag)
                            <li class="mb-2"><a href="{{route('frontend.shop_tag', $shop_tag->slug)}}" class="reset-anchor">{{ $shop_tag->name}}</a></li>
                        @empty
                        @endforelse
                    </ul>
                        ...
                        <div class="col-lg-6">
                            <ul class="list-inline d-flex align-items-center justify-content-lg-end mb-0">
                                ...
                                <li class="list-inline-item text-muted mr-3">
                                    <a class="reset-anchor p-0" href="javascript:void(0);" id="three_items"><i class="fas fa-th"></i>
                                    </a>
                                </li>
